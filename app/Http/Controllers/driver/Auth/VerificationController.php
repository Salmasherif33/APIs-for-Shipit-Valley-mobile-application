...
class VerificationController extends Controller
{
    ...
    */
    ...
    public function verify(Request $request)
    {
        ...
        if (request('phone')) {
            ...
            if (request('code') == Session::where('drivers_id', '=', $driver->id)->latest()->first()->code) {
                Session::where('drivers_id', '=', $driver->id)->latest()->first()->delete();
    ...
    public function forgotPasswordVerify(Request $request)
    {
        ...
        $id =  auth('driver_api')->user()->id;


        if (
            Session::where('drivers_id', '=', $id)->where('tmp_code', '=', $TmpToken)->latest()->first()->code
            == request('code')
        ) {
            return response([
                'status' => 200,
    ...
    public function verifyNewPhone(Request $request)
    {
        ...
        if ($request->code == Session::where('drivers_id', '=', $driver->id)->latest()->first()->code) {
            $driver->phone = Session::where('drivers_id', '=', $driver->id)->latest()->first()->tmp_phone;
            $driver->save();
    ...
    public function resend(Request $request){
        ...
        $session = Session::where('drivers_id','=',$driver->id);
        if($session->count() > 0){
